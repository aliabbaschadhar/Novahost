...
export async function initKafkaConsumer() {
  await consumer.connect()
    ...
    .catch((err) => console.error("Error happened while connection consumer:", err))

  await consumer.subscribe({ topics: [`container-logs`] })
    .then(() => (
      console.log("Successfully subscribed to container-logs")
    ))
    .catch((err) => (
      console.error("Unable to subscribe to topics:", err)
    ))
